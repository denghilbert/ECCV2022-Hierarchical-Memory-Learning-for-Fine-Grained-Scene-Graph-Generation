...
def stage_train(cfg, local_rank, distributed, logger):
    ...
    parent_importance = None  # stage00 01 02 03 04 10 11 12 13 14累计的importance
    ...
    for stage in range(4):
        ...
        if distributed:
            ...
            if stage == 0:
                stage_modules[1].module.cpu()
                stage_modules[2].module.cpu()
                stage_modules[3].module.cpu()
                torch.cuda.empty_cache()
            elif stage == 1:
                stage_modules[0].module.cuda()
                stage_modules[1].module.cuda()
                stage_modules[2].module.cpu()
                stage_modules[3].module.cpu()
                torch.cuda.empty_cache()
            elif stage == 2:
                stage_modules[0].module.cpu()
                stage_modules[3].module.cpu()
                stage_modules[1].module.cuda()
                stage_modules[2].module.cuda()
                torch.cuda.empty_cache()
            else:
                stage_modules[0].module.cpu()
                stage_modules[1].module.cpu()
                stage_modules[2].module.cuda()
                stage_modules[3].module.cuda()
                torch.cuda.empty_cache()
        else:
        ...
        son_theta_last_iter = None  # 用于存放上一个batch(iter)的loss用于计算importance score
        son_theta_current_iter = None  # 当前son本次iter的param的

        son_fisher_last_iter = None  # 用于在stage2中上一个iter的fisher
        son_fisher_current_iter = None  # 用于计算在stage2中本次iter的fisher

        # 假设一个stage 5个iter
        son_importance_current_iter = None  # stage 21 22累计的importance
        son_importance_last_iter = None  # stage 21累计的importance

        loss_last_iter = None
        loss_current_iter = None

        for iteration, (images, targets, _) in enumerate(train_data_loader[stage], start_iter[stage]):

            # 读取之前保存好的一些模型参数
            # 在训练过程中就没必要在读取一次了
            if son_importance_last_iter == None and son_fisher_last_iter == None and son_theta_last_iter == None:
                # 训练的开始也没必要
                # 并且没必要存储theta因为模型本身自带了这个东西可以后面直接读出来
                if stage > 0:
                    parent_importance = torch.load(
                        cfg.OUTPUT_DIR + "/param_distill/parent_importance_stage" + str(stage - 1) + ".pytorch").cuda()
                    ...
                    if iteration != 0:
                        son_importance_last_iter = torch.load(
                            cfg.OUTPUT_DIR + "/param_distill/son_importance_current_iter(stage" + str(
                                stage) + "iter" + str(iteration) + ").pytorch").cuda()
            ...
            if stage == 0:
                ...
                loss_current_iter = loss_dict['loss_rel']
            else:
                ...
                # 计算parameter_loss
                # parent_fisher = parent_fisher.cuda()
                # parent_importance = parent_importance.cuda()
                # parent_theta = parent_theta.cuda()
                # 本次训练的参数储存
                ...
                # 选择蒸馏拟合参数的方式
                """
                print(torch.mean(parent_fisher))
                print(torch.mean(parent_importance))
                print(torch.mean(torch.abs(parent_theta - theta_now)))
                print(torch.mean((parent_theta - theta_now)**2))
                print(torch.mean(cfg.SOLVER.LAMBDA_FOR_PARAM * (parent_fisher + parent_importance) * (torch.abs(parent_theta - theta_now))))
                print(torch.mean(cfg.SOLVER.LAMBDA_FOR_PARAM * (parent_fisher + parent_importance) * ((parent_theta - theta_now)**2)))
                ...
                # tensor(39.5317, device='cuda:0', grad_fn= < MeanBackward0 >)
                # tensor(8.9903, device='cuda:0', grad_fn= < MeanBackward0 >)
                """

                if cfg.SOLVER.DISTILL_TYPE == 'abs':
                    loss_dict["loss_param_distill"] = torch.mean(
                        cfg.SOLVER.LAMBDA_FOR_PARAM * (parent_fisher + parent_importance) * (
                            torch.abs(parent_theta - theta_now)))
                else:
                    loss_dict["loss_param_distill"] = torch.mean(
                        cfg.SOLVER.LAMBDA_FOR_PARAM * (parent_fisher + parent_importance) * (
                                    (parent_theta - theta_now) ** 2))

                del theta_now

                # parent_fisher = parent_fisher.cpu()
                # parent_importance = parent_importance.cpu()
...
# --------------------------------backward之后有梯度就可以算fisher了------------------------------------#
            ...
            # 本次训练的参数储存
            son_theta_last_iter = torch.tensor([]).cuda()
            for name, param in stage_modules[stage].named_parameters():  # 把本阶段的参数储存起来
                if 'roi_heads.relation.predictor.context_layer' in name or \
                        'roi_heads.relation.predictor.post' in name or \
                        'roi_heads.relation.predictor.rel' in name or \
                        'roi_heads.relation.predictor.ctx' in name:
                    son_theta_last_iter = torch.cat((son_theta_last_iter, param.view(-1)))
            # 为了节约，每次操作完了都放到cpu上面去
            # son_theta_last_iter = son_theta_last_iter.cpu()

            # backward之后才有梯度，所以fisher的计算和更新都是在后面进行的
            son_fisher_current_iter = torch.tensor([]).cuda()
            grad_current_iter = torch.tensor([]).cuda()
            for name, param in stage_modules[stage].named_parameters():
                if 'roi_heads.relation.predictor.context_layer' in name or \
                    ...
                    if param.grad != None:
                        ...
                        grad_current_iter = torch.cat((grad_current_iter, param.grad.view(-1)))

                    else:
                        son_fisher_current_iter = torch.cat(
                            (son_fisher_current_iter, torch.tensor([0 for i in range(len(param.view(-1)))]).cuda()))
                        grad_current_iter = torch.cat(
                            (grad_current_iter, torch.tensor([0 for i in range(len(param.view(-1)))]).cuda()))
            ...
            if son_fisher_last_iter == None:
                ...
                # print("fisher")
                # print(son_fisher_last_iter[60688: 60720])
                # print(son_fisher_current_iter[60688: 60720])
                # son_fisher_last_iter = son_fisher_last_iter.cuda()
            ...
            # 节约显存
            # son_fisher_current_iter = son_fisher_current_iter.cpu()
            # son_fisher_last_iter = son_fisher_last_iter.cpu()
            # grad_current_iter = grad_current_iter.cpu()
            # 让nvidia-smi可以显示可用显存
            torch.cuda.empty_cache()
# --------------------------------backward之后有梯度就可以算fisher了------------------------------------#
...
# -----------------------------------step之后模型参数才进行更新----------------------------------------#
            ...
            # step之后记录本次循环更新之后的参数
            son_theta_current_iter = torch.tensor([]).cuda()
            for name, param in stage_modules[stage].named_parameters():  # 把本阶段的参数储存起来
                if 'roi_heads.relation.predictor.context_layer' in name or \
                        'roi_heads.relation.predictor.post' in name or \
                        'roi_heads.relation.predictor.rel' in name or \
                        'roi_heads.relation.predictor.ctx' in name:
                    son_theta_current_iter = torch.cat((son_theta_current_iter, param.view(-1)))

            # 实际上真正开始计算importance_score是从iter=1开始的
            # 虽然theta最开始没训练的有一个，但是loss并不是所以需要往后移一个位置
            if iteration != 1:
                # 利用这个batch更新好的fisher/loss变化/theta变化计算这个stage中的importance_scores
                delta_loss = torch.tensor(1, dtype=torch.float).cuda() if loss_last_iter - loss_current_iter > 0 else torch.tensor(0, dtype=torch.float).cuda()
                # 移动回到显存里面
                # son_theta_last_iter = son_theta_last_iter.cuda()
                delta_theta = son_theta_current_iter - son_theta_last_iter
                # 节约显存
                # son_theta_last_iter = son_theta_last_iter.cpu()
                # torch.cuda.empty_cache()
                # son_theta_current_iter = son_theta_current_iter.cpu()
                # torch.cuda.empty_cache()
                # son_fisher_current_iter = son_fisher_current_iter.cuda()
                # grad_current_iter = grad_current_iter.cuda()

                """
                这是我新改进的importance计算的方法
                print("分母")
                denominator = son_fisher_current_iter * delta_theta**2
                denominator[denominator < 1e-20] = 1e-20
                print("max" + str(torch.max(denominator)))
                print("min" + str(torch.min(denominator)))
                print("mean" + str(torch.mean(denominator)))

                print("分子")
                numerator = grad_current_iter * delta_theta
                numerator[numerator < 0] = 0
                numerator[numerator < 1e-15] = 1e-15
                print("max" + str(torch.max(numerator)))
                print("min" + str(torch.min(numerator)))
                print("mean" + str(torch.mean(numerator)))

                son_importance_current_iter = 2 * numerator / denominator
                print("重要")
                print(torch.max(son_importance_current_iter))
                print(torch.min(son_importance_current_iter))
                print(torch.mean(son_importance_current_iter))
                """

                # 这是最开始我写的importance的计算公式
                # son_importance_current_iter = (2 * (grad_current_iter * delta_theta) / (son_fisher_current_iter * delta_theta**2))
                # # 清理importance中nan，太大变成1e+20，小于1e+12的值变成1e+12
                # son_importance_current_iter[son_importance_current_iter.ne(son_importance_current_iter)] = 0
                # son_importance_current_iter[son_importance_current_iter < 1e+12] = 1e+12
                # son_importance_current_iter[son_importance_current_iter > 1e+20] = 1e+20
                ...
                # numerator = -delta_loss * numerator
                ...
                # grad_current_iter = grad_current_iter.cpu()
                # son_fisher_current_iter = son_fisher_current_iter.cpu()
                # delta_theta = delta_theta.cpu()
                # torch.cuda.empty_cache()

                # 更新importance_scores
                if son_importance_last_iter == None:
                    son_importance_last_iter = son_importance_current_iter
                else:
                    # print("importance update")
                    # print(son_importance_last_iter[60688: 60720])
                    # print(son_importance_current_iter[60688: 60720])
                    # son_importance_last_iter = son_importance_last_iter.cuda()
                    son_importance_current_iter += son_importance_last_iter.detach()
                    # print(son_importance_current_iter[60688: 60720])

                    # 把当前的importance_scores储存起来
                    son_importance_last_iter = son_importance_current_iter

                    # son_importance_current_iter = son_importance_current_iter.cpu()
                    # son_importance_last_iter = son_importance_last_iter.cpu()

            # 记录本次iter的loss
            loss_last_iter = loss_dict['loss_rel']

            # if iteration > 1:
            #     print(son_theta_current_iter[: 10])
            #     print(son_theta_last_iter[: 10])
            #     print(son_importance_current_iter[: 10])
            #     print(son_importance_last_iter[: 10])
            #     print(son_fisher_current_iter[: 10])
            #     print(son_fisher_last_iter[: 10])
            # 让nvidia-smi把空余的显存显示出来
            torch.cuda.empty_cache()

            """用于生成测试的三个parent参数"""
            # if iteration == 10:
            #     print(son_importance_current_iter / torch.sum(son_importance_current_iter))
            #     print(son_theta_current_iter)
            #     print(son_fisher_current_iter / torch.sum(son_fisher_current_iter))
            #     torch.save(son_importance_current_iter / torch.sum(son_importance_current_iter), cfg.OUTPUT_DIR + "/param_distill/parent_importance_stage0.pytorch")
            #     torch.save(son_theta_current_iter, cfg.OUTPUT_DIR + "/param_distill/parent_theta_stage0.pytorch")
            #     torch.save(son_fisher_current_iter / torch.sum(son_fisher_current_iter),cfg.OUTPUT_DIR + "/param_distill/parent_fisher_stage0.pytorch")
# -----------------------------------step之后模型参数才进行更新----------------------------------------#
            ...
            if iteration % checkpoint_period == 0 or iteration == 7990:
                ...
                assert np.nan not in (son_fisher_current_iter) and \
                       np.nan not in (son_importance_current_iter) and \
                       np.inf not in (son_importance_current_iter) and \
                       np.inf not in (son_fisher_current_iter)
                torch.save(son_fisher_current_iter,
                           cfg.OUTPUT_DIR + "/param_distill/son_fisher_current_iter(stage" + str(stage) + "iter" + str(
                               iteration) + ").pytorch")
                torch.save(son_importance_current_iter,
                           cfg.OUTPUT_DIR + "/param_distill/son_importance_current_iter(stage" + str(
                               stage) + "iter" + str(iteration) + ").pytorch")
...
# --------------------------------一个stage结束后需要进行的操作--------------------------------------#
        ...
        if son_fisher_current_iter != None and son_importance_current_iter != None:
            assert np.nan not in (son_fisher_current_iter) and \
                   np.nan not in (son_importance_current_iter) and \
                   np.inf not in (son_importance_current_iter) and \
                   np.inf not in (son_fisher_current_iter)
            ...
            parent_theta = parent_theta.cpu()
            # 这了储存进行了平均是因为放置随着数字的累加爆炸
            if parent_importance == None:
                son_importance_current_iter = torch.log10(son_importance_current_iter.detach())
                son_importance_current_iter = son_importance_current_iter - son_importance_current_iter.mean()
                parent_importance = 1.0 / (1.0 + torch.exp(-son_importance_current_iter))
            else:
                son_importance_current_iter = torch.log10(son_importance_current_iter.detach())
                son_importance_current_iter = son_importance_current_iter - son_importance_current_iter.mean()
                parent_importance = ((1.0 / (
                            1.0 + torch.exp(-son_importance_current_iter))) + parent_importance.detach()) / 2
            parent_importance = parent_importance.cpu()
            ...
            assert np.nan not in (son_fisher_current_iter) and \
                   ...
                   np.inf not in (son_fisher_current_iter)
            # 保存起来后面万一要用
            torch.save(parent_importance,
                       cfg.OUTPUT_DIR + "/param_distill/parent_importance_stage" + str(stage) + ".pytorch")
        ...
        # 删除本阶段的所有变量
        del son_theta_last_iter
        del son_theta_current_iter
        del son_fisher_last_iter
        del son_fisher_current_iter
        del son_importance_current_iter
        del son_importance_last_iter
        del loss_last_iter
        del loss_current_iter
# --------------------------------一个stage结束后需要进行的操作--------------------------------------#
