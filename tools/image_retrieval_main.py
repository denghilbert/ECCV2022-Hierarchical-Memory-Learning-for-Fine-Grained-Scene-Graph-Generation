...
from maskrcnn_benchmark.image_retrieval.model import SGEncode
...
test_path = '/media/data1/deng_sgg/image_retrieval/vctree_test.json'
train_path = '/media/data1/deng_sgg/image_retrieval/vctree_train.json'
def read_sg(test_path, train_path):
    with open(test_path, 'r')as f:
        test = json.load(f)
        test_ids = test[0].keys()
    with open(train_path, 'r')as f:
        train = json.load(f)
        train_ids = train[0].keys()
    merge = train[0].copy()
    merge.update(test[0])
    return merge, list(train_ids), list(test_ids)

sg_model_name = 'motif'
sg_fusion_name = 'rubi'
sg_type_name = 'origin'

sg_train_path = '/media/data1/deng_sgg/image_retrieval/vctree_train.json'
sg_test_path = '/media/data1/deng_sgg/image_retrieval/vctree_test.json'
output_path = '/media/data1/deng_sgg/image_retrieval/vctree/image_retrieval_model/vctree_output.pytorch'

# sg_train_path = '/media/data1/deng_sgg/image_retrieval/causal_{}_sgdet_{}_{}_train.pytorch'.format(sg_model_name, sg_fusion_name, sg_type_name)
# sg_test_path = '/media/data1/deng_sgg/image_retrieval/causal_{}_sgdet_{}_{}_test.pytorch'.format(sg_model_name, sg_fusion_name, sg_type_name)
# output_path = '/media/data1/deng_sgg/image_retrieval/image_retrieval_model/causal_'+sg_model_name+'_sgdet_'+sg_fusion_name+'_'+sg_type_name+'_output_%s_%d.pytorch'
...
sg_data, train_ids, test_ids = read_sg(test_path, train_path)
# <class 'dict'>
# 13203
# <class 'dict'>
# 28699
...
def train(cfg, local_rank, distributed, logger):
    ...
    for epoch in range(cfg.SOLVER.MAX_ITER):
        ...
        for iteration, (fg_imgs, fg_txts, bg_imgs, bg_txts) in enumerate(train_data_loader):
            ...
            count_for_print += 1
            print('\r 一个epoch进行了... %.2f %%' % (count_for_print / len(train_data_loader) * 100), end='')
