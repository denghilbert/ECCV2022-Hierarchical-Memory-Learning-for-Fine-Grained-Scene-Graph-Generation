...
class RelationFeatureExtractor(nn.Module):
    ...
    def forward(self, x, proposals, rel_pair_idxs=None):
        # print("在roi_box_feature_extractors里面")
        # print(proposals)
        ...
        for proposal, rel_pair_idx in zip(proposals, rel_pair_idxs):
            ...
            # print("head tail rect")
            # print(len(rel_pair_idx[:, 0]))
            # print(rel_pair_idx[:, 1])
            # print(head_proposal)
            # print(tail_proposal)
            # print(head_rect.shape)
            # print(tail_rect.shape)
            # print(rect_input.shape)
            # sum += rect_input.shape[0]
            # head_proposal = 物体个数*物体个数 - 物体个数（自己之间没有关系）
            # tail_proposal 同理
