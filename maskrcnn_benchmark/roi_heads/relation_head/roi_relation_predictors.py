...
@registry.ROI_RELATION_PREDICTOR.register("dengtest")
class dengtest(nn.Module):
    def __init__(self, config, in_channels):
        super(dengtest, self).__init__()
        ...
        self.cos_normal = config.MODEL.USE_COS_NORMALIZATION
        # load class dict
        ...
        # print(TransformerContext(config, obj_classes, rel_classes, in_channels))
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        #     print(rel_pair_idxs[i].shape)
        #     sum_of_rel += rel_pair_idxs[i].shape[0]
        # print(sum_of_rel)
        # print("rel_binarys")
        # sum_of_object = 0
        # for i in range(len(rel_binarys)):
        #     print(rel_binarys[i].shape)
        #     sum_of_object += rel_binarys[i].shape[0]
        # print(sum_of_object)
        # print(roi_features.shape)
        # print(union_features.shape)
        """
        proposals
        [BoxList(num_boxes=8, image_width=600, image_height=800, mode=xyxy), BoxList(num_boxes=5, image_width=600, image_height=900, mode=xyxy), BoxList(num_boxes=10, image_width=600, image_height=800, mode=xyxy), BoxList(num_boxes=26, image_width=600, image_height=800, mode=xyxy), BoxList(num_boxes=4, image_width=600, image_height=600, mode=xyxy), BoxList(num_boxes=9, image_width=600, image_height=800, mode=xyxy), BoxList(num_boxes=6, image_width=600, image_height=895, mode=xyxy), BoxList(num_boxes=5, image_width=600, image_height=840, mode=xyxy), BoxList(num_boxes=10, image_width=600, image_height=800, mode=xyxy), BoxList(num_boxes=12, image_width=600, image_height=600, mode=xyxy), BoxList(num_boxes=5, image_width=600, image_height=900, mode=xyxy), BoxList(num_boxes=9, image_width=600, image_height=600, mode=xyxy), BoxList(num_boxes=8, image_width=600, image_height=826, mode=xyxy), BoxList(num_boxes=8, image_width=600, image_height=869, mode=xyxy), BoxList(num_boxes=6, image_width=600, image_height=800, mode=xyxy), BoxList(num_boxes=10, image_width=600, image_height=602, mode=xyxy)]
        rel_pair_idxs
        torch.Size([56, 2])
        torch.Size([20, 2])
        torch.Size([90, 2])
